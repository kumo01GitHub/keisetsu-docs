...
以下は、上記の docs 運用で参照するサービス側の共通コンセプトです。

## コンセプト

keisetsu は、学習者が学習データを自分で保持し、いつでも学べる状態を守るための OSS 単語帳です。

- 必要性
  - データの非公開化、オンライン依存、更新経路の不透明さといった既存課題を解消する
- 優位性
  - `schema.sql` と `catalog` を公開し、`.kdb`（SQLite）で可搬性を持たせ、検証スクリプトで配布品質を担保する
- 実現方針
  - 公開スキーマ + 公開データ + オフライン学習を軸に、制作（admin）・配布（database）・利用（mobile）を分離する
- 目指す未来
  - プラットフォームに依存しない学習基盤を作り、学習者・教材制作者・開発者・教育現場が共通契約上で継続利用と拡張を行える状態にする
