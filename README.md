...
- [keisetsu-publisher](https://github.com/kumo01GitHub/keisetsu-publisher)
    ...
    - deck manifest（JSON）と .kdb を同梱した ZIP バンドルのみ出力
...
- [keisetsu-mobile](https://github.com/kumo01GitHub/keisetsu-mobile)
    ...
    - kdbダウンロード / ローカルkdbファイル取り込み
    - 学習モード・テストモード
    - テスト履歴保存、デッキ切替・削除

## 標準ワークフロー

1. `keisetsu-publisher` でカードを作成・編集し、deck manifest（JSON）と .kdb を同梱した ZIP を生成する
2. ZIP を展開し、`.kdb` を `keisetsu-database/databases/`、manifest（{deck-id}.json）を `keisetsu-database/catalog/decks/` に配置する
3. `keisetsu-database` で `npm run build` を実行し、`catalog/catalog.json` を再生成する
4. `keisetsu-database` で `npm run validate:kdb` および `npm run validate:catalog` で形式・整合性を検証する
...
5. ダウンロードした ZIP を展開し、生成物を keisetsu-database に配置して、[追加・編集申請](https://github.com/kumo01GitHub/keisetsu-database/pulls)
